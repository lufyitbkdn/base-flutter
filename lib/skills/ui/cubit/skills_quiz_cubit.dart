...
class SkillsQuizCubit extends Cubit<SkillsQuizState> {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      if (hasCompletedSkillsQuiz) {
        ...
        emit(SkillsQuizState.startTakingSkillsQuiz(skillQuestions: skillsRepository.skillQuestions));
      }
    } catch (e, s) {
  ...
  void handleRetakeTestSelected() => emit(SkillsQuizState.startTakingSkillsQuiz(skillQuestions: skillsRepository.skillQuestions));

  void goToNextQuestion() {
    final state = this.state;
    if (state is! SkillsQuizStateTakingSkillsQuiz) {
      throw StateError("Can't go to the next question if the user is not taking the quiz");
    }

    final nextQuestionIndex = currentQuestionIndex + 1;
    final isNextQuestionTheLast = nextQuestionIndex == state.scoredQuestions.length - 1;
  ...
  void changeQuestionScore(int newScore) {
    final state = this.state;
    if (state is! SkillsQuizStateTakingSkillsQuiz) {
      throw StateError('The question score should only change if the user is taking the quiz');
    }
  ...
  Future<void> saveQuestions() async {
    final state = this.state;
    if (state is! SkillsQuizStateTakingSkillsQuiz) {
      throw StateError('The scores should only be saved if the user is taking the quiz');
    }
...
sealed class SkillsQuizState with _$SkillsQuizState {
  ...
  const factory SkillsQuizState.hasTakenSkillsQuiz() = SkillsQuizStateHasTakenSkillsQuiz;
  ...
  factory SkillsQuizState.startTakingSkillsQuiz({required List<SkillQuestion> skillQuestions}) {
  ...
  const factory SkillsQuizState.savingSkillsQuizResults() = SkillsQuizStateSavingSkillsQuizResults;
