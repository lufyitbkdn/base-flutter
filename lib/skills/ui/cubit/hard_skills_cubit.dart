...
class HardSkillsCubit extends Cubit<HardSkillsState> {
  HardSkillsCubit({required this.skillsRepository}) : super(const HardSkillsState.loading());
  ...
  Future<void> load() async {
    ...
    try {
      final selectedRankedHardSkill = await skillsRepository.loadRankedHardSkill();
      ...
      log.severe('Error while trying to load HardSkillsCubit', error, stacktrace);
  ...
  Future<void> save(RankedHardSkill rankedHardSkill) async {
    ...
    try {
      ...
      log.severe('Error while trying to save the Ranked Hard Skill', error, stacktrace);
      emit(const HardSkillsState.error());
    }
  }
}
