...
import 'package:civstart/skills/data/models/skills_data.dart';
import 'package:civstart/skills/ui/cubit/hard_skills_cubit.dart';
...
class _HardSkillsPageState extends State<HardSkillsPage> {
  ...
  Future<void> selectHardSkill(List<RankedHardSkill> rankedHardSkills, RankedHardSkill? selectedRankedHardSkill) async {
    var selectedSkillIndex = selectedRankedHardSkill == null ? 0 : rankedHardSkills.indexOf(selectedRankedHardSkill);
        ...
        children: [
          PaddedContent(
            children: [
              ...
              CivText.bodyMedium('Hard skills are specific abilities, capabilities and skill sets that an individual can possess and demonstrate in a measured way. Hard skills are learnable skills that enable individuals to perform job-specific tasks, or that may be required for a specific job.'),
              const SizedBox(height: Spacing.s_16),
              BlocBuilder<HardSkillsCubit, HardSkillsState>(
                bloc: bloc,
                builder: (context, state) {
                  return switch (state) {
                    HardSkillsStateLoaded(:final rankedHardSkills, :final selectedRankedHardSkill) => Column(
                        children: [
                          Center(
                            child: selectedRankedHardSkill == null
                                ? CivButton.primaryOutlined(
                                    title: 'Select your rank',
                                    onPressed: () => selectHardSkill(rankedHardSkills, selectedRankedHardSkill),
                                  )
                                : TextButton(
                                    onPressed: () => selectHardSkill(rankedHardSkills, selectedRankedHardSkill),
                                    child: CivText.titleMedium(
                                      selectedRankedHardSkill.rank,
                                      mergeStyleWith: TextStyle(color: Theme.of(context).primaryColor),
                                    ),
                                  ),
                          ),
                          const SizedBox(height: Spacing.s_16),
                          Center(
                            child: selectedRankedHardSkill == null //
                                ? CivText.bodyMedium('Select your rank to see a list of skills that apply to your experience in defence.')
                                : RankedSkillDescription(rankedHardSkill: selectedRankedHardSkill),
                          ),
                        ],
                      ),
