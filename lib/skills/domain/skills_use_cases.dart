...
class HasCompletedSkillsQuizUseCase {
  ...
  Future<bool> call() async {
    final softSkillsQuizResultsData = await skillsRepository.loadSoftSkillsQuizResult();
...
class SortSkillQuizResultsUseCase {
  ...
  Future<SortedSkillsQuizResult?> call() async {
    ...
    final List<Skill> sortedSkills = groupBy(softSkillsQuizResultsData, (scoredQuestion) => scoredQuestion.skillQuestion.skill)
        .map((key, scoredQuestions) {
          return MapEntry(
            key,
            scoredQuestions.fold(0, (previousSum, scoredQuestion) => scoredQuestion.score + previousSum),
          );
        })
        .entries
    ...
    if (sortedSkills.length < 6) {
      throw StateError('There should be more than 6 skills, but there are only ${sortedSkills.length}');
    }

    final topSkills = sortedSkills.sublist(0, 3)..forEach(sortedSkills.remove);
    final lowerSkills = sortedSkills.sublist(sortedSkills.length - 3)..forEach(sortedSkills.remove);
...
class SortedSkillsQuizResult with _$SortedSkillsQuizResult {
  ...
  factory SortedSkillsQuizResult.fromJson(Map<String, Object?> json) => _$SortedSkillsQuizResultFromJson(json);
}
