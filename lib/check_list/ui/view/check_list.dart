          ...
          builder: (context, state) {
            return switch (state) {
              CheckListStateLoaded(:final checkList) => CheckList(
                  checkList: checkList,
                  indexedValueChanged: (value, index) => widget.bloc.changeIsCheckedForIndex(index: index, isChecked: value),
                ),
  ...
  MaterialStateBorderSide? getSide(BuildContext context) {
    return MaterialStateBorderSide.resolveWith((Set<MaterialState> states) {
      ...
      if (states.contains(MaterialState.disabled)) {
        ...
        return BorderSide(width: 2.0, color: colorScheme.primary.withOpacity(0.38));
      }
      if (states.contains(MaterialState.selected)) {
