...
class CheckListCubit extends Cubit<CheckListState> {
  CheckListCubit({required this.checklistRepository}) : super(const CheckListState.loading());
  ...
  Future<void> changeIsCheckedForIndex({required int index, required bool isChecked}) async {
  ...
  Future<void> resetAll() async {
    final state = this.state;
    if (state is CheckListStateLoaded) {
      final resetedCheckList = state.checkList.map((checkListItem) => checkListItem.copyWith(isChecked: false)).toList();
      try {
...
sealed class CheckListState with _$CheckListState {
  ...
  const factory CheckListState.loaded({required List<CheckListItem> checkList}) = CheckListStateLoaded;
}
