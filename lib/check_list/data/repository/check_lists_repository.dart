...
final class CheckListsRepository {
  ...
  @visibleForTesting
  Future<Map<String, List<CheckListItem>>> get checkLists async => checkListsMemo.runOnce(loadAll);

  @visibleForTesting
  Future<Map<String, List<CheckListItem>>> loadAll() async {
    final checkListDataString = await localStorageDataSource.read(StorageKey.checklist);
  ...
  Future<void> saveCheckList(List<CheckListItem> checkList, String id) async {
    ...
    await localStorageDataSource.write(key: StorageKey.checklist, value: jsonCheckListString);
  }

  @protected
  Future<List<CheckListItem>?> loadById(String id) async => (await checkLists)[id];
}
...
abstract class CheckListRepository {
  ...
  Future<List<CheckListItem>?> load() async => checkListsRepository.loadById(checkListId);

  Future<void> save(List<CheckListItem> checkList) async => checkListsRepository.saveCheckList(checkList, checkListId);
